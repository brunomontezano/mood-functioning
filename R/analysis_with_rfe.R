...
# Create the objects
mood_train_before_rfe <- rsample::training(mood_split)
mood_test_before_rfe <- rsample::testing(mood_split)


# Fill missing values only to do RFE

df_removed <- mood_train_before_rfe %>% select(outcome, rec)


library(mice)
ds_imputed <- mice(mood_train_before_rfe %>% select(-outcome, -rec), 
                   m = 5, 
                   maxit = 50, 
                   method = "pmm", 
                   seed = 1)
...
ds_completed <- data.frame(df_removed, ds_completed)
...
rfeProfile <- rfe(x = ds_completed %>% select(-outcome),
                 y = ds_completed$outcome, 
...
mood_train <- mood_train_before_rfe %>% select(all_of(c("outcome", "rec", bestVar)))
mood_test <- mood_test_before_rfe %>% select(all_of(c("outcome", "rec", bestVar)))
