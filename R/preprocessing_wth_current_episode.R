...
raw %>% select(perdas,
  ...
  miniA08AT_t2,
    ...
    # Nem mania atual
    # Do not have current manic episode
    miniD07AT_t2) %>% mutate_all(as.factor) %>% summary()
...
subsample$FAST_sum_t2
subsample %>% select(FAST_sum_t2)
...
save.image("session/preprocessing_with_current_episode.RData")

saveRDS(ds,
        file = "cache/dataset_with_current_episode.rds")

write.csv(ds, file = "cache/ds_current_episodes_with_missings.csv")
